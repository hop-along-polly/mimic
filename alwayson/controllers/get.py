...
async def get(req: Request, pathname: str, manifests: Annotated[dict, Depends(inject_manifest_repo)]) -> JSONResponse:
  request = AlwaysOnRequest(
    ...
    url=str(req.url)  # TODO Consider only using a path and not the full url i.e `/health` instead of `http://localhost:/health`
  )

  responses, calls = await manifests.get_responses(request)

  if not responses or len(responses) <= calls:
    return JSONResponse({ 'message': f'/{pathname} has not been configured with a response' }, 400)
