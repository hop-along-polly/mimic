...
from alwayson.controllers.manifests import get_manifest_repo
...
async def test_upload_full_manifest(mock_db, mock_manifest_repo):
  ...
  app.dependency_overrides[get_manifest_repo] = mock_manifest_repo

  payload = [
    {
      'request': {
        'method': 'GET',
        'url': 'https://testerozza.com/health'
      },
      'responses': [{
        'status_code': 200,
        'body': { 'status': 'healthy' }
  ...
  assert AlwaysOnRequest(method='GET', url='https://testerozza.com/health') == actual.request
  assert [AlwaysOnResponse(status_code=200, body={ 'status': 'healthy' })] == actual.responses
