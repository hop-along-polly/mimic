...
async def test_get_endpoint_single_call(mock_manifest_repo):
  ...
  # Act
  subject.post('/v1/manifests', json=test_manifest)  # Upload a Manifest. This is likely how customers would be writing their tests
...
async def test_get_endpoint_not_configured(mock_manifest_repo):
  ...
  assert { 'message': '/health has not been configured with a response' } == actual.json()
